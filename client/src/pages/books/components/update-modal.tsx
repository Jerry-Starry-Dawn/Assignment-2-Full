...
import { Book, UpdateBook } from '@/types/book'
...
function UpdateModal({
  ...
  const [ytdSales, setYtdSales] = useState<number>(book.ytdSales ?? 0)
  const [publisher, setPublisher] = useState<string>(
    book.publisher.id.toString() ?? ''
  )
  const [royalty, setRoyalty] = useState<number>(book.royalty ?? 0)
  const [notes, setNotes] = useState<string>(book.notes ?? '')
  const [price, setPrice] = useState<number>(book.price ?? 0)
  const [advanced, setAdvanced] = useState<number>(book.advance ?? 0)
  const [date, setDate] = useState<Date>(
    book.publishedDate ? new Date(book.publishedDate) : new Date()
  )
  console.log(book)
  const [selectedAuthors, setSelectedAuthors] = useState<string[]>(
    book.bookAuthors.map((a) => a.authorId.toString())
  )
  ...
  const handleCreateAuthor = async () => {
    ...
    const res = await bookService.updateBook(book.id, data)
    if (res.data) {
      onClose()
      navigate(0)
    }
